...
class typogen(object):
    ...
    def generatetyposv2(self, strHost, strCountry="gb", bTypos=True, iTypoIntensity=100, bTLDS=False, bBitFlip=True,
        ...
        if bBitFlip:
            temp = self.bitflipstring(strHost)
            for eiei in temp:
                print("bitflipstring",eiei,strHost)

        if bTypos:
            #Quick:
            temp = self.generate_missing_character_typos(strHost)
            for eiei in temp:
                print("missing_character",eiei,strHost)
            temp= self.generate_duplicate_character_typos(strHost)
            for eiei in temp:
                print("duplicate_character",eiei,strHost)
            temp = self.generate_ings_and_plurals(strHost)
            for eiei in temp:
                print("ings_and_plurals",eiei,strHost)
            temp = self.generate_replace_i_l_1_o_0(strHost)
            for eiei in temp:
                print("replace_i_l_1_o_0",eiei,strHost)
            temp = self.generate_ings_and_plurals_then_replace_i_l_1_o_0(strHost)
            for eiei in temp:
                print("ings_and_plurals_then_replace_i_l_1_o_0",eiei,strHost)

            # Balanced:
            if iTypoIntensity > 0:
                temp = self.generate_miskeyed_typos(strHost, strCountry)
                for eiei in temp:
                    print("miskeyed",eiei,strHost)
                temp = self.generate_miskeyed_sequence_typos(strHost, strCountry)
                for eiei in temp:
                    print("miskeyed_sequence",eiei,strHost)
            
            # Rigorous phase:
            if iTypoIntensity > 50:
                temp = self.generate_transposed_character_typos(strHost)
                for eiei in temp:
                    print("transposed_character",eiei,strHost)
                temp = self.generate_miskeyed_addition_typos(strHost, strCountry)
                for eiei in temp:
                    print("miskeyed_addition",eiei,strHost)
        ...
        if bHomoglyphs:
            temp = self.generate_homoglyph_confusables_typos(strHost)
            for eiei in temp:
                print("homoglyph_confusables",eiei,strHost)
            temp = self.generate_additional_homoglyph_typos(strHost)
            for eiei in temp:
                print("additional_homoglyph",eiei,strHost)
        ...
        if bDoppelganger:
            ...
            temp = self.generate_subdomain_doppelgangers(strHost)
            for eiei in temp:
                print("subdomain_doppelgangers",eiei,strHost)
            temp = self.generate_extra_dot_doppelgangers(strHost)
            for eiei in temp:
                print("extra_dot_doppelgangers",eiei,strHost)
